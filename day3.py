        ...
        for row_c in range(max(row_i - 1, 0), min(row_i + 2, row_len)):
            ...
            if element != '.' and not element.isnumeric():
                ...
                if element == '*':
                    ...
                    matrix[row_c][column_c] = 'G_2 ' + \
                        matrix[row_c][column_c] + ' ' + str(number)
        ...
        for row_i, line in enumerate(matrix):
            ...
            while column_i < column_len:
                ...
                if line[column_i].isnumeric():
                    ...
                    sum += adjacency_matrix(matrix, row_i,
                                            column_i, column_e, row_len, column_len)
                    column_i = column_e
                column_i += 1
        print(sum)
        ...
        for line in matrix:
            for char in line:
                if char.startswith('G_2'):
                    number_1, number_2 = int(char.split(
                        ' ')[2]), int(char.split(' ')[3])
